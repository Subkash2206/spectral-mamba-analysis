...
The "Spectral Debt" hypothesis as a global predictor of boundary failure is **refuted**. Spectral aliasing is an architectural trait, but not the primary driver of boundary precision across different paradigms.

### C. Shift Consistency (Robustness)
Mamba maintains superior shift consistency compared to Transformers, losing only 2.2% IoU under pixel translations, approaching CNN-level stability.

#### Translation Equivariance Results (Mean IoU)
| Architecture | 1px Shift | 3px Shift | 5px Shift | Drop |
| :--- | :---: | :---: | :---: | :---: |
| **UNet-ResNet50** | 0.9864 | 0.9678 | **0.9693** | **-1.7%** |
| **VM-UNet (Mamba)** | 0.9755 | 0.9551 | 0.9532 | **-2.2%** |
| **Swin-Tiny** | 0.9670 | 0.9484 | 0.9196 | -4.9% |
...
## 4. Reproducibility
1. `tools/boundary_eval.py`: Rigorous validation on ISIC18.
2. `avr_stagewise_all.py`: Unified stage-wise spectral audit.
3. `shift_consistency.py`: Translation equivariance testing.
4. `VM-UNet/visualizations.py`: Figure generation suite.