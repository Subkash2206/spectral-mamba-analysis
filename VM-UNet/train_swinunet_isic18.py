...
class ISICDataset(Dataset):
    def __init__(self, img_paths, mask_dir, size=224, is_train=True):
...
def main():
    ...
    train_dataset = ISICDataset(train_imgs, mask_dir, size=224, is_train=True)
    val_dataset = ISICDataset(val_imgs, mask_dir, size=224, is_train=False)
    ...
    # The user asked to use pretrained weights from pretrained_weights/swin_tiny_patch4_window7_224.pth
    # Let's dynamically update the config
    config.defrost()
    config.MODEL.PRETRAIN_CKPT = 'pretrained_weights/swin_tiny_patch4_window7_224.pth'
    config.freeze()
    
    model = SwinUnet(config, img_size=224, num_classes=1).to(device)
    model.load_from(config) # This loads the backbone weights internally in Swin-Unet
    ...
    for epoch in range(1, epochs + 1):
        ...
        if val_dice > best_val_dice:
            ...
            torch.save(model.state_dict(), 'best-ckpt/best-swinunet-isic18.pth')
            
        with open('swinunet_progress.txt', 'a') as f:
