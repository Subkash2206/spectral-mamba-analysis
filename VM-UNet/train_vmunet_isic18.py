...
def main():
    ...
    print("Loading VM-UNet model with pretrained weights...")
    model = VMUNet(load_ckpt_path='pre_trained_weights/pre_trained_weights/vmamba_tiny_e292.pth').to(device)
    model.load_from() # Load the pretrained backbone weights
    
    # DiceLoss with from_logits=False because model applies sigmoid internally
    ...
    for epoch in range(1, epochs + 1):
        ...
        for images, masks in train_loader:
            ...
            optimizer.zero_grad()
            outputs = model(images) # model already applies sigmoid
        ...
        with torch.no_grad():
            for images, masks in val_loader:
                ...
                val_dice += compute_dice(outputs, masks).item() * images.size(0)
        val_dice /= len(val_loader.dataset)
        
        if val_dice > best_val_dice:
            best_val_dice = val_dice
            torch.save(model.state_dict(), 'best-ckpt/best-vmunet-scratch-isic18.pth')
            
        with open('vmunet_progress.txt', 'a') as f:
